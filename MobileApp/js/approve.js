    ...
    $.ajax({
        ...
        $("#list-table tbody a").click(function (e) {
            ...
            window.location.href = url.filename + (url.search ? url.search + "&" : "?")
                + "&workflowType=" + $("#workflowType").val() + "&objectId=" + $("#objectId").val() + "&workflowId=" + $("#workflowId").val() + "#detail";
        });
...
var getListDataCallBack = function (index, Element) {
    ...
    var html = '<tr>' +
        '<th>' + Element.Id + '</th>' +
        '<td class="title"><a href="#" wid="' + Element.AppObjId + '" oid="' + Element.Id + '" wtype="' + Element.ObjType + '">' + Element.AppObjName + '</a></td>' +
