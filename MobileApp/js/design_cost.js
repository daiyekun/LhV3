 ...
 * @type {string}
 */
var businessTypeDisplayName = "设计变更费用签证单";
...
var getListData = function (page, dataUrl, callback) {
    ...
    $.ajax({
        ...
        $("#list-table tbody a").click(function (e) {
            ...
            window.location.href = "design_cost.html?id=" + this.id + "#detail";
        });
...
$(document).on("pageinit", "#index", function () {
    ...
    var dataUrl = sysUrl + woo.url.designCostList;
...
$(document).on("pageshow", "#detail", function () {
    ...
    if (detailId > 0) {
        //获取基本信息
        $.ajax({
            url: sysUrl + woo.url.designCostDetail,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
            ...
            console.log("获取设计变更费用签证单基本信息失败!");
        });

        //获取设计变更单附件信息
        $.ajax({
            url: sysUrl + woo.url.designCostDetailFiles,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
            ...
            console.log("获取设计变更费用签证单附件信息失败!");
        });

        //获取设计变更单审批记录信息
        $.ajax({
            url: sysUrl + woo.url.designCostDetailWorkflow,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
            ...
            console.log("获取设计变更费用签证单审批记录信息失败!");
        });
    }
...
$(document).on("pageshow", "#detail_updateState", function () {
    ...
    form.unbind('submit').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: sysUrl + woo.url.designCostState,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
