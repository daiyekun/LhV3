 ...
 * @type {string}
 */
var businessTypeDisplayName = "合同外委托单";
...
var getListData = function (page, dataUrl, callback) {
    ...
    $.ajax({
        ...
        $("#list-table tbody a").click(function (e) {
            ...
            window.location.href = "outsourced.html?id=" + this.id + "#detail";
        });
...
$(document).on("pageinit", "#index", function () {
    ...
    var dataUrl = sysUrl + woo.url.outSourcedList;
...
$(document).on("pageshow", "#detail", function () {
    ...
    if (detailId > 0) {
        //获取基本信息
        $.ajax({
            url: sysUrl + woo.url.outSourcedDetail,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
            ...
            d.find('[name="gcnr"]').html(r.gcnr);
            d.find('[name="gcl"]').html(r.gcl);
            d.find('[name="wtyy"]').html(r.wtyy);
            ...
            console.log("获取合同外委托单基本信息失败!");
        });

        //获取合同外委托单附件信息
        $.ajax({
            url: sysUrl + woo.url.outSourcedDetailFiles,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
            ...
            console.log("获取合同外委托单附件信息失败!");
        });

        //获取合同外委托单审批记录信息
        $.ajax({
            url: sysUrl + woo.url.outSourcedDetailWorkflow,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
            data: {
...
$(document).on("pageshow", "#detail_updateState", function () {
    ...
    form.unbind('submit').submit(function (e) {
        e.preventDefault();
        $.ajax({
            url: sysUrl + woo.url.outSourcedState,
            type: "POST",
            dataType: "jsonp",
            crossDomain: "true",
