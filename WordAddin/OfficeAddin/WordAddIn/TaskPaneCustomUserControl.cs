...
namespace WordAddIn
{
    public partial class TaskPaneCustomUserControl : UserControl
    {
       ...
       private WordAddinUtility addinUtility = new WordAddinUtility();
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void listboxSysVar_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                //设计模式

                var selitem = (ContractVariable)this.listboxSysVar.SelectedItem;
                foreach (ContentControl contentControl in WordShare.WordApp.ActiveDocument.ContentControls)
                {
                    if (contentControl.Type == WdContentControlType.wdContentControlRichText && selitem.VarName == contentControl.Tag)
                    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void listboxCustomVar_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                //设计模式

                var selitem = (ContractVariable)this.listboxCustomVar.SelectedItem;
                foreach (ContentControl contentControl in WordShare.WordApp.ActiveDocument.ContentControls)
                {
                    if (contentControl.Type == WdContentControlType.wdContentControlRichText && selitem.VarName == contentControl.Tag)
                    {
