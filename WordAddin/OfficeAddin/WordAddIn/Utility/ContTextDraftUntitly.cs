...
namespace WordAddIn.Utility
{
    ...
    public class ContTextDraftUntitly
    {
        ...
        public int ShowSubjectMatter(Word.ContentControl Control)
        {
            ...
            try
            {
                ...
                if (tabscount > 0)
                {
                    ...
                    if (Control.Tag == "27")
                    {
                        rng.InsertBefore(listtabObj[0].DETAIL_TITLE);
                        rng.Font.Name = "宋体";
                        rng.Font.Size = 10;
                        ...
                        var tableIndex = 0;
                        //创建表格
                        CreateSubTable(rng, ref missing, listtabObj, tabscount, ref tableIndex);
                    }
        ...
        private void CreateSubTable(Word.Range rng, ref System.Object missing, IList<ContractObjectTable> listtabObj, int tabscount, ref int tableIndex)
        {
            try
            {
                ...
                foreach (var info in listtabObj)
                {
                    ...
                    if (rowcount > 0)
                    {
                        ...
                        else
                        {
                            ...
                            Word.Range tableLocation = WordShare.WordApp.ActiveDocument.Range(ref end, ref end);
        ...
        private  void SetTableData(JArray array, int rowcount, int cellcount, Word.Table tbl,int ShowNumRow)
        {
        ...
        private void CreateSubMatterDetailTable(JArray array, int rowcount, int cellcount, Word.Table tbl)
        {
            //标的明细显示
            for (int r = 1; r <= rowcount; r++)
            {
                for (int c = 1; c <= cellcount; c++)
                {
                    if (r < rowcount)
                    {
                        ...
                        tbl.Cell(r, c).VerticalAlignment = Word.WdCellVerticalAlignment.wdCellAlignVerticalCenter;//居中
                    }
                    else
                    { //合计行
        ...
        /// <param name="tbl">Table表格</param>
        private void CreateSubMatterMarkTable(JArray array, int rowcount, int cellcount, Word.Table tbl)
        {
            for (int r = 1; r <= rowcount; r++)
            {
                for (int c = 1; c <= cellcount; c++)
        ...
        private  void SetGaiYaoXinXi(JArray array, int rowcount, int cellcount, Word.Table tbl, int r, int c)
        {
        ...
        /// <param name="r">当前行</param>
        /// <param name="c">当前列</param>
        private  void ShowLastRowMerge(JArray array, int cellcount, Word.Table tbl, int r, int c)
        {
            switch (c)
            {
                case 1:
