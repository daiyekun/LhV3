...
namespace WordAddIn.ClassLib.WordAddinClass
{
    ...
    public class ContractFinalProcess:WordAddinBase
    {
       ...
       /// </summary>
        ...
        {
            foreach (MsWord.ContentControl nativeControl in document.ContentControls)
            {
                ...
                else if (nativeControl.Type == Microsoft.Office.Interop.Word.WdContentControlType.wdContentControlRichText)
                {
                    nativeControl.LockContentControl = false;
                    nativeControl.LockContents = false;
                    nativeControl.Range.Shading.BackgroundPatternColor = MsWord.WdColor.wdColorWhite;
        ...
        public string ConvertPDFToVsto(string templatePath, string savePath, string contextPath)
        {
            ...
            MsWord.Document docNewWord = null;
            try
            {
                ...
                MsWord.Document docWaterMark = wordApp.Documents.Add(templatePath);

                //使用word模板添加构建基块
                MsWord.Template template = (MsWord.Template)docNewWord.get_AttachedTemplate();
                MsWord.Range templateHeaderRange = docNewWord.Sections[1].Headers[MsWord.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range;
                MsWord.WdParagraphAlignment headerAlignment = templateHeaderRange.ParagraphFormat.Alignment;
                MsWord.Range waterHeaderRange = docWaterMark.Sections[1].Headers[MsWord.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range;
                MsWord.BuildingBlock waterMark = template.BuildingBlockEntries.Add("newWaterMark", MsWord.WdBuildingBlockTypes.wdTypeWatermarks, "General", waterHeaderRange);

                //插入已添加到模板的水印生成块
                templateHeaderRange.Collapse(MsWord.WdCollapseDirection.wdCollapseEnd);
                waterMark.Insert(templateHeaderRange, true);



                //页眉格式调整
                MsWord.Range rg = docNewWord.Sections[1].Headers[MsWord.WdHeaderFooterIndex.wdHeaderFooterPrimary].Range;
                var rText = rg.Text.Replace("\r", "").Replace("\n", "");
                var leng = rText.Length;
                if (leng > 0)
                {
                    docNewWord.Sections[1].Borders[MsWord.WdBorderType.wdBorderBottom].LineStyle = MsWord.WdLineStyle.wdLineStyleNone;
                    rg.ParagraphFormat.Alignment = headerAlignment;
                    rg.Collapse(MsWord.WdCollapseDirection.wdCollapseEnd);
                    rg.MoveStart(MsWord.WdUnits.wdCharacter, 1);
                    rg.Delete(MsWord.WdUnits.wdCharacter, 1);
                }

                docNewWord.Save();
                docNewWord.ActiveWindow.View.Type = MsWord.WdViewType.wdNormalView;
                docNewWord.ActiveWindow.View.Type = MsWord.WdViewType.wdPrintView;
                docNewWord.ActiveWindow.View.ShowRevisionsAndComments = false;
                docNewWord.ExportAsFixedFormat(savePath, MsWord.WdExportFormat.wdExportFormatPDF);
                return "ok";
            }
            catch (Exception e)
            {
            ...
            finally
            {
                if (docNewWord != null)
                {
                    object notsavechange = MsWord.WdSaveOptions.wdDoNotSaveChanges;
