...
namespace WordAddIn2010.Unttitily
{
    ...
    public class WordReviewUtility
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static IList<MsWord.Comment> GetComments(WordTool.Document wordDoc) 
        {
            IList<MsWord.Comment> listComment = new List<MsWord.Comment>();
            foreach (MsWord.Comment wordComment in wordDoc.Comments)
            {
        ...
        public static void SetCommentRange(WordTool.Document document)
        {
            ...
            foreach (var comminfo in listComment)
            {
                ...
                MsWord.Range rage = comminfo.Range;
                var text = rage.Text;
                ...
                if (author != WordShare.UserName)//表示不是当前用户新增批注
                {
                    ...
                    foreach (MsWord.ContentControl nativeControl in rage.ContentControls)
                    {
                        if (nativeControl.Type == Microsoft.Office.Interop.Word.WdContentControlType.wdContentControlRichText)
                        {
                            ...
                            if (controlTxt != text)
                            {
                                ...
                                comminfo.Range.Text = "";
                                //创建控件
                                MsWord.ContentControl newNativeControl = comminfo.Range.ContentControls.Add();
        ...
        public static void DeleteComments(IList<MsWord.Comment> listComment)
        {
        ...
        #region 新增批注
        public static void OnNewComment(Office.IRibbonControl control)
        {
            
            WordShare.WordApp.ActiveDocument.CommandBars["Reviewing"].Controls["批注(&M)"].Execute();
        }
        #endregion
        ...
        public static void SetDocCommentEditState()
        {
            ...
            foreach (var cominfo in listComment)
            {
                ...
                MsWord.Range rage = cominfo.Range;
                var text = rage.Text;
                ...
                if (cominfo.Author != WordShare.UserName)
                {
                    ...
                    MsWord.ContentControl control = CreateCommentControl(document, author, rage, text, _index);
                ...
                {
                    MsWord.ContentControl control = null;
                    var count = rage.ContentControls.Count;
                    if (count > 0)
                    {
                        object cont_Index = 0;
        ...
        public static MsWord.ContentControl CreateCommentControl(WordTool.Document document, string author, MsWord.Range rage, string text, int _index)
        {
            ...
            MsWord.ContentControl contentControl = rage.ContentControls.Add();
            //设置控件空值时显示内容
