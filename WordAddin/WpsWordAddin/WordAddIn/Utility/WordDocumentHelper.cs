...
namespace WordAddIn.Utility
{
    ...
    public class WordDocumentHelper
    {
        ...
        {
            MsWord.Document currentDocument = Globals.ThisAddIn.Application.ActiveDocument;
        ...
        public static void SetRichTextContentValue(IList<ContractVariable> ListData, bool IsSP = false,bool IsEditWord=false)
        {
            ...
            try
            {
                    ...
                    if (nativeControl.Type == Microsoft.Office.Interop.Word.WdContentControlType.wdContentControlRichText)
                    {
                        if (dict.ContainsKey(nativeControl.Tag))
                        {
                            if (nativeControl.Tag != "27" && nativeControl.Tag != "38")
                            {
                                ...
                                nativeControl.Range.Shading.BackgroundPatternColor = MsWord.WdColor.wdColorViolet;
                                //锁定内容
                            ...
                            {//标的明细和标的概要都执行
                                ...
                                #region 启动一个线程去画表格，wps 目前没解决多线程问题 
                                //启动一个线程去画表格，wps 目前没解决多线程问题
                        ...
                        {
                            nativeControl.Range.Shading.BackgroundPatternColor = MsWord.WdColor.wdColorYellow;
                            nativeControl.LockContentControl = true;
                            nativeControl.LockContents = false;
                ...
                if (!dict.ContainsKey("27") && !dict.ContainsKey("38"))
                {
                    ...
                    if (IsSP)
                    {
                        DocProtectOrUnProtect(0, MsWord.WdProtectionType.wdAllowOnlyComments);
                    }
                    else
                    {
                        // DocProtectOrUnProtect(0);
            ...
            catch (Exception ex)
            {
                ...
                LogUtility.WriteLog(typeof(WordDocumentHelper), ex);
            }
        ...
        public static void SetRichTextContentValue(bool IsReView)
        {
            ...
            foreach (MsWord.ContentControl nativeControl in document.ContentControls)
            {
                if (nativeControl.Type == Microsoft.Office.Interop.Word.WdContentControlType.wdContentControlRichText)
                {
                    ...
                    nativeControl.Range.Shading.ForegroundPatternColor = MsWord.WdColor.wdColorWhite;
                    nativeControl.LockContentControl = true;
                    nativeControl.LockContents = true;
        ...
        private static MsWord.WdColor GetControlBak(int IsCustomerVar)
        {
                ...
                case requestAddinWordType.TextDraft:
                    {
                        if (IsCustomerVar == 1)
                        { //系统变量
                            return MsWord.WdColor.wdColorViolet;
                        }
                        else
                        { //自定义变量
                            return MsWord.WdColor.wdColorYellow;
                        }


                    }
                default:
                    return MsWord.WdColor.wdColorWhite;
        ...
        /// </param>
        public static void DocProtectOrUnProtect(int State = 0, MsWord.WdProtectionType protType = MsWord.WdProtectionType.wdAllowOnlyComments)
        {      
        ...
        public static void DocProtectOrUnProtectSPEdit(int State = 0, MsWord.WdProtectionType protType = MsWord.WdProtectionType.wdAllowOnlyRevisions)
        {
            Object missing = Type.Missing;
            Object Password = WordShare.WordPassword;
            if (State == 0)
            ...
            else
            {
                ...
                if(WordShare.WordApp.ActiveDocument.ProtectionType != MsWord.WdProtectionType.wdAllowOnlyRevisions)
                {
        ...
        /// 比如：MsWord.WdProtectionType.wdAllowOnlyFormFields 标示文档锁定了，但是文档内容控件是可以编辑的
        /// </param>
        public  void DocProtectOrUnProtect2(int State = 0, MsWord.WdProtectionType protType = MsWord.WdProtectionType.wdAllowOnlyComments)
        {
        ...
        private static void DocProtect(MsWord.WdProtectionType protType, ref Object missing, ref Object Password)
        {
            object objFalse = true;

            if (WordShare.WordApp.ActiveDocument.ProtectionType != MsWord.WdProtectionType.wdNoProtection)
            {
