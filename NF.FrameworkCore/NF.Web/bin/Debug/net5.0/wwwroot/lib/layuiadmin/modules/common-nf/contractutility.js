   ...
   , admin = layui.admin
    ...
    var contractutility = {
                            ...
                            case "0"://当前状态“未执行”
                                {
                                    if (checkData[0].ModificationTimes > 0) {
                                        $(".datastate").append('<dd id="zxz"lay-event="stateChange" flowitem="2" tostate="2"><i  class="layui-icon layui-icon-edit"></i>变更未执行-->执行中</dd>');
                                        $(".datastate").append('<dd lay-event="stateChange" flowitem="5" tostate="3"><i class="layui-icon layui-icon-edit"></i>未执行-->已作废</dd>');
                                    } else {
                                        $(".datastate").append('<dd id="zxz" lay-event="stateChange" flowitem="1" tostate="1"><i  class="layui-icon layui-icon-edit"></i>未执行-->执行中</dd>');
                                    $(".datastate").append('<dd lay-event="stateChange" flowitem="5" tostate="3"><i class="layui-icon layui-icon-edit"></i>未执行-->已作废</dd>');
                                    }
                                }
                                break;
        ...
        updateSate01: function (param) {
            ...
            var checkStatus = table.checkStatus(param.tableId)
                , checkData = checkStatus.data; //得到选中的数据
            var state = param.sZT;// $(param.evtobj).attr("tostate");//修改状态
            var flowitem = param.sZT;// $(param.evtobj).attr("flowitem");
            var _userId = layui.data(setter.tableName, { key: setter.NFData.userId });
            var _Id = param.Sid;
            /**
            *去请求流程
