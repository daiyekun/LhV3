            ...
            try
            {
                ...
                switch (InportType)
                {
                    case 1://合同对方
                        {
                            ...
                            var list = ImportDataHelperData.ImportCompany(path, listdics, users, gj, Sf, sj);
                    ...
                    case 2://合同{
                        {
                            ...
                            var listcmp = _ICompanyService.GetQueryable(a => a.IsDelete != 1).Select(a => new NF.ViewModel.Models.CompanyInfo { Id = a.Id, Name = a.Name }).ToList();
                            var listbz = _ICurrencyManagerService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportContract(path, listdics, users, depts, listcmp, listbz, Xminfo);
                    ...
                    case 3://项目
                        {
                            ...
                            //var depts = _IDepartmentService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var listbz = _ICurrencyManagerService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportProj(path, listdics, users, listbz);
                    ...
                    case 4://计划资金
                        {
                            ...
                            var list = ImportDataHelperData.ImportPlanFince(path, listdics, users, listhts, listbz);
                    ...
                    case 5://实际资金导入
                        {
                            ...
                            var listbz = _ICurrencyManagerService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var listhts = _IContractInfoService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportContAcfince(path, listdics, users, listhts, listbz);
                    ...
                    case 6://发票
                        {
                            ...
                            var listbz = _ICurrencyManagerService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var listhts = _IContractInfoService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportInvoice(path, listdics, users, listhts, listbz);
                    ...
                    case 7://标的
                        {
                            ...
                            var listbz = _ICurrencyManagerService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var listhts = _IContractInfoService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportSubMit(path, listdics, users, listhts, listbz);
                    ...
                    case 8://用户
                        {
                            var depts = _IDepartmentService.GetQueryable(a => a.IsDelete != 1).ToList();
                            var list = ImportDataHelperData.ImportUserInfo(path, depts);
