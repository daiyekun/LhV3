            ...
            admin.req({
                ...
                data: {
                    ...
                    if (res.Data.Isktz == 0) {
                        ...
                        document.getElementById("sxz").style.display = "";
                        $("#Isktz0").attr("checked", true);
                    } else if (res.Data.Isktz == 1) {//审批规则
                     
                        document.getElementById("sxz").style.display = "none";
                        $("#Isktz1").attr("checked", true);
                    }
    ...
    var active = {
        ...
        addUsers: function () {
            ...
            layer.open({
                type: 2
                , title: '选择组'
                , content: '/WorkFlow/GroupInfo/SelectGroupusinfo?Zid='+Zid
                , maxmin: true
                , area: ['60%', '90%']
                , btn: ['确定', '取消']
                , btnAlign: 'c'
                ...
                , yes: function (index, layero) {
                    ...
                    for (var i = 0; i < checkData.length; i++) {
                        if (usname == "") {
                            usname = checkData[i].Name;
                        } else {
                            usname = usname +"," +checkData[i].Name;
                        }
                        if (usid == "") {
                            usid = checkData[i].Id;
                        } else {
                            usid = usid + "," + checkData[i].Id;
                        }
                    ...
                    var sdr = $("#NodeStrId").val();
                 
                    console.log(sdr)
                    $("#GroupName").val(checkData[0].GroupNames);
                    $("#groUserNames").text(usname);
                    $("#GroupId").val(checkData[0].GroupId);
                    $("#UsIds").val(usid);
                    active.saveNodeInfo();
                    layer.close(index);
                    return false;
