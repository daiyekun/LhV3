...
function bindStyle(svgEl, style, el) {
    ...
    if (pathHasFill(style)) {
        var fill = (0, core_1.normalizeColor)(style.fill);
    ...
    if (pathHasStroke(style)) {
        var stroke = (0, core_1.normalizeColor)(style.stroke);
        ...
        var lineDash = style.lineDash && strokeWidth > 0 && (0, dashStyle_1.normalizeLineDash)(style.lineDash, strokeWidth);
        if (lineDash) {
...
var svgPath = {
    brush: function (el) {
        ...
        if (!svgEl) {
            svgEl = (0, core_1.createElement)('path');
            el.__svgEl = svgEl;
        }
        if (!el.path) {
            el.createPathProxy();
...
var svgImage = {
    brush: function (el) {
        ...
        var svgEl = el.__svgEl;
        if (!svgEl) {
            svgEl = (0, core_1.createElement)('image');
            el.__svgEl = svgEl;
        }
...
var svgText = {
    brush: function (el) {
        ...
        if (!textSvgEl) {
            textSvgEl = (0, core_1.createElement)('text');
        ...
        var x = style.x || 0;
        var y = adjustTextY(style.y || 0, (0, text_1.getLineHeight)(font), style.textBaseline);
