...
exports.brush = exports.brushSingle = exports.createCanvasPattern = void 0;
...
function createCanvasPattern(ctx, pattern, el) {
    var image = (0, image_1.createOrUpdateImage)(pattern.image, pattern.__image, el);
    if ((0, image_1.isImageReady)(image)) {
...
function brushPath(ctx, el, style, inBatch) {
    ...
    if (!inBatch) {
        ...
        if (hasFillGradient) {
            fillGradient = el.__dirty
                ? (0, helper_1.getCanvasGradient)(ctx, fill, rect)
                : el.__canvasFillGradient;
            el.__canvasFillGradient = fillGradient;
        }
        if (hasStrokeGradient) {
            strokeGradient = el.__dirty
                ? (0, helper_1.getCanvasGradient)(ctx, stroke, rect)
    ...
    var lineDash = style.lineDash && style.lineWidth > 0 && (0, dashStyle_1.normalizeLineDash)(style.lineDash, style.lineWidth);
    ...
    if (lineDash) {
        ...
        if (lineScale_1 && lineScale_1 !== 1) {
            lineDash = (0, util_1.map)(lineDash, function (rawVal) {
    ...
    if (firstDraw || (el.__dirty & constants_1.SHAPE_CHANGED_BIT)
...
function brushImage(ctx, el, style) {
    var image = el.__image = (0, image_1.createOrUpdateImage)(style.image, el.__image, el, el.onload);
    if (!image || !(0, image_1.isImageReady)(image)) {
        return;
    }
    var x = style.x || 0;
    var y = style.y || 0;
...
function bindCommonProps(ctx, style, prevStyle, forceSetAll, scope) {
    ...
    if (forceSetAll || style.opacity !== prevStyle.opacity) {
        ...
        ctx.globalAlpha = isNaN(opacity) ? Displayable_1.DEFAULT_COMMON_STYLE.opacity : opacity;
    }
...
function brush(ctx, el, scope, isLast) {
    ...
    if (!el.shouldBePainted(scope.viewWidth, scope.viewHeight, false, false)) {
        el.__dirty &= ~constants_1.REDRAW_BIT;
    ...
    if (!prevElClipPaths || (0, helper_1.isClipPathChanged)(clipPaths, prevElClipPaths)) {
    ...
    if (el instanceof Path_1["default"]) {
        ...
        else if (el instanceof IncrementalDisplayable_1["default"]) {
